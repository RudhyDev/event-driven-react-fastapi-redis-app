...
const Delivey = (props) => {
  ...
  const submit = async (e, type) => {
    ...
    const response = await fetch('http://localhost:8000/deliveries/event', {
      method: 'POST',
  ...
  return <div className='row w-100'>
    ...
    <div className="col-12 mb-5">
      <div className="progress">
        {state.status !== 'ready' ? <div className={state.status === 'active' ? "progress-bar bg-success progress-bar-striped progress-bar-animated" : "progress-bar bg-success "} role="progressbar" style={{ width: '50%' }} ></div> : ''}
        {state.status === 'collected' || state.status === 'completed' ? <div className={state.status === 'collected' ? "progress-bar bg-success progress-bar-striped progress-bar-animated" : "progress-bar bg-success"} role="progressbar" style={{ width: '50%' }} ></div> : ''}
      </div>
    </div>
    <div className="col-3">
      <div className='card'>
